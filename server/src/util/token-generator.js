...
function generateAccessToken() {
    return randLetterSeq(4) + generateToken() + randLetterSeq(6) + 
            randCharSeq(6) + generateToken() + randCharSeq(6);
}

/**
 * Generates a random sequence of characters, possibly empty.
 *
 * @param {number} max Maximum length of the string that we want.
 * @returns {string} Random sequence, possibly empty.
 */
function randCharSeq(max = 5) {
    let toReturn = "";
    const toAdd = randInt(0, max + 1);
    for (let j = 0; j < toAdd; j++) {
        toReturn += randChar();
